{if isset($skip_pot) && $skip_pot}
    <script type="text/javascript">
        ...
        $.ajax({
            url: "{$urls.base_url}module/tunnelventeabies/{if isset($back) && $back}boule?back=1{else}autresapin{/if}",
            type: "post",
            data: 'ajax=1&pot=0',
            dataType: 'json',
...
{else}
    <form action="{$urls.base_url}module/tunnelventeabies/autresapin" id="form_pot"
          method="post">
        <h4>{l s="Choisissez le cahe pot" mod='tunnelventeabies'}</h4>
        ...
        <div class="list_pot">
            {foreach from=$result item=pot}
                ...
                <label for="not_pot">{l s="Aucun pot disponible"  mod='tunnelventeabies'}</label>
            ...
            {if count($result) }
                ...
                <input type="radio" name="pot" id="not_pot" class="radio_pot" data-price="0" value="0"
                       data-image="{$urls.base_url}modules/tunnelvente/images/pot/default-pot.png"/>
                <label for="not_pot">{l s="Je ne souhaite pas ajouter de cache pot"  mod='tunnelventeabies'}</label>
                <img style="display: none;"
                     src="{$urls.base_url}modules/tunnelvente/images/pot/default-pot.png"/>
            </span>
            {/if}
    ...
    <script type="text/javascript">
        var baseurl_tunnelvente = "{$urls.base_url}module/tunnelventeabies/boule?back=1";
    </script>
{literal}
    <script type="text/javascript">
        $(function ($) {
