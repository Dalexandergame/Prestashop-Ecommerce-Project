...
{if isset($orderProducts) && count($orderProducts) > 0}
	<div class="crossseling-content">
			...
			<ul id="blockcart_caroucel">
				{foreach from=$orderProducts item='orderProduct' name=orderProduct}
					<li>
						...
						<div class="text-cross pull-left"><p class="product-name">
        ...
        <script type="text/javascript">
            $(function($){
                $('#blockcart_caroucel li a').click(function(event){
                    event.preventDefault();
                    $('#layer_cart .cross').click();
                    if($('.choix_spain').length && !$('#steps').is(':visible'))
                        setTimeout(function(){ $('.choix_spain').click(); }, 700);  
                })
            });
        </script>
{/if}