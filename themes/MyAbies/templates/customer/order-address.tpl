...
{else}
	...
	<h1 class="page-heading step-num"><b><span>01 /</span> {l s='Addresses'}</b></h1>
	<div id="opc_account" class="opc-main-block">
		<div id="opc_account-overlay" class="opc-overlay" style="display: none;"></div>
{/if}
<div class="addresses clearfix">

{assign var="hasValidAddress" value="false"}
{if is_numeric($cart->npa)}
    {foreach from=$addresses key=k item=address}
        {if $cart->npa == $address.postcode}
            {assign var="hasValidAddress" value="true"}
            {break}
        {/if}
    {/foreach}
{/if}

{*test_{if is_numeric($cart->npa)}yes{else}no{/if}*}
	{if $hasValidAddress == "true" }
	<div class="row">
		...
		<div class="col-xs-12 col-sm-6">
			<div id="address_invoice_form" class="select form-group selector1"{if $cart->id_address_invoice == $cart->id_address_delivery} style="display: none;"{/if}>
				...
				{else}
					<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1&select_address=1{if $back}&mod={$back}{/if}")|escape:'html':'UTF-8'}" title="{l s='Add'}" class="btn btn-default btn-add">
						<span>
	...
	{elseif !is_numeric($cart->npa) && $addresses|@count > 0 }
	<div class="row">
		<div class="col-xs-12 col-sm-6">
			<div class="address_delivery select form-group selector1">
				<label for="id_address_delivery">{if $cart->isVirtualCart()}{l s='Choose a billing address:'}{else}{l s='Choose a delivery address:'}{/if}</label>
	...
	<div class="alert alert-danger">
			{l s='Votre adresse de livraison ne correspond pas au NPA de la commande Merci de mettre à jour votre adresse de livraison ou de contacter le service client'}
    </div>
	{/if}
	<p class="address_add submit">
		<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{if $back}&mod={$back}{/if}")|escape:'html':'UTF-8'}" title="{l s='Add'}" class="btn btn-default btn-add">
...
{*<script>
    $("document").ready(function(){
        $('html,body').animate({
            scrollTop: $("#opc_account").offset().top-100
        });
    });
</script>*}