...
<div class="box box-small clearfix">
	<form id="submitReorder" action="{if isset($opc) && $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" class="submit">
			...
			<input type="hidden" value="" name="submitReorder"/>
				...
				<a href="#" onclick="$(this).closest('form').submit(); return false;" class="btn link-command pull-right"><span>{l s='Reorder'}<!-- <i class="icon-angle-right right"></i> --></span></a>
			<p class="dark">
...
<div class="table_block">
	<table class="detail_step_by_step table">
		<thead>
			<tr>
		...
		<tfoot>
			{if $priceDisplay && $use_tax}
				<tr class="item">
					...
					<td class="text-right" colspan="{if $order->hasProductReturned()}5{else}4{/if}">
						<span class="price price1">{displayWtPriceWithCurrency price=$order->getTotalProductsWithoutTaxes() currency=$currency}</span>
					</td>
				</tr>
			{/if}
			<tr class="item">
				...
				<td class="text-right" colspan="{if $order->hasProductReturned()}5{else}4{/if}">
					<span class="price price2">{displayWtPriceWithCurrency price=$order->getTotalProductsWithTaxes() currency=$currency}</span>
				</td>
			</tr>
			...
			<tr class="totalprice item">
				...
				<td class="text-right" colspan="{if $order->hasProductReturned()}5{else}4{/if}">
					<span class="price price3">{displayWtPriceWithCurrency price=$order->total_paid currency=$currency}</span>
				</td>
			</tr>
		</tfoot>
		<tbody>
			...
			{if !isset($product.deleted)}
				...
				{if $product.product_quantity > $product.customizationQuantityTotal}
					<tr class="item">
						...
						<td><label class="color-myaccount" for="cb_{$product.id_order_detail|intval}">{if $product.product_reference}{$product.product_reference|escape:'html':'UTF-8'}{else}--{/if}</label></td>
						<td class="bold">
	...
	<table class="table footab">
		...
		<tbody>
			...
			<tr class="item">
				...
				<td data-value="{if $order->getTaxCalculationMethod() == $smarty.const.PS_TAX_INC}{$line.shipping_cost_tax_incl}{else}{$line.shipping_cost_tax_excl}{/if}" class="shipping-price">{if $order->getTaxCalculationMethod() == $smarty.const.PS_TAX_INC}{displayPrice price=$line.shipping_cost_tax_incl currency=$currency->id}{else}{displayPrice price=$line.shipping_cost_tax_excl currency=$currency->id}{/if}</td>
				<td>
...
{if !$is_guest}
	...
	<form action="{$link->getPageLink('order-detail', true)|escape:'html':'UTF-8'}" method="post" class="std" id="sendOrderMessage">
		<p class="carrier_title norm">{l s='Add a message'}</p>
		<p class="text_carrier">{l s='If you would like to add a comment about your order, please write it in the field below.'}</p>
		<p class="form-group">
		...
		<div class="submit">
			...
			<button type="submit" name="submitMessage" class="btn btn-add"><span>{l s='Send'}<!-- <i class="icon-chevron-right right"></i> --></span></button>
		</div>
	</form>
{else}
