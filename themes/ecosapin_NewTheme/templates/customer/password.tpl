...
<div class="box col-md-6" id="pass-box">
...
<form action="{$request_uri|escape:'html':'UTF-8'}" method="post" class="std" id="form_forgotpassword">
	<fieldset>
		<div class="form-group">
			...
			<input placeholder="{l s='Email address'}" class="form-control" type="text" id="email" name="email" value="{if isset($smarty.post.email)}{$smarty.post.email|escape:'html':'UTF-8'|stripslashes}{/if}" />
		</div>
		<p class="submit text-right">
            <button type="submit" class="btn btn-default button" id="SubmitLogin"><span>{l s='Retrieve Password'}<!-- <i class="icon-chevron-right right"></i> --></span></button>
		</p>
	</fieldset>
</form>
{/if}
