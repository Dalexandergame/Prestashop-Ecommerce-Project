...
class Hashing
{
     ...
     */
    public function isFirstHash($passwd, $hash, $staticSalt = COOKIE_KEY)
    {
     ...
     */
    public function checkHash($passwd, $hash, $staticSalt = COOKIE_KEY)
    {
        if (!count($this->hashMethods)) {
            $this->initHashMethods();
        }
     ...
     * @return string
     */
    public function hash($plaintextPassword, $staticSalt = COOKIE_KEY)
    {
        if (!count($this->hashMethods)) {
            $this->initHashMethods();
        }
