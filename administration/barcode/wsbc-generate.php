...
//    printf($_sql);exit;
    ...
    foreach ($orders as $id_order => $order) {
        ...
        foreach ($productsSapinsOnly as $product) {
            ...
            $productName = ucfirst(mb_substr($product['product_reference'], 0, 7)) . '.';
            ...
            $productName = wd_remove_accents($productName) . $deco_pot_str . "" . (isset($order['hasRetour']) ? ".R" : "");
            ...
            for ($i = 0; $i < $product['product_quantity']; $i++) {
                $item = array(// 1.Item ...
                              ...
                              'Recipient'  => array(
                                  'Title'   => mb_substr(str_replace(' ', '.', $productName), 0, 35),
