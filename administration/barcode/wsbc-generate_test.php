...
$decoration_et_pot = [52, 53];
$id_produts_genereEtiqueete = array_merge($id_products_sapin,$id_product_mylittel,$decoration_et_pot);
 ...
 echo "test2";
    foreach ($orders as $id_order => $order) {
        ...
        foreach ($order['products'] as $product) {
            if(in_array($product['product_id'], $decoration_et_pot) ){
                $deco_pot_str .= substr(explode(':',$product['product_name'])[1],0,6);
            }else{
                $productsSapinsOnly[] = $product;
            }
        }

 echo "test3";
        foreach ($productsSapinsOnly as $product) {
            ...
            $productName = ucfirst(substr($product['product_reference'],0,7)).'.';
            ...
            $productName = wd_remove_accents($productName). $deco_pot_str . "" . (isset($order['hasRetour']) ? ".R." : "")."" . date("d/m", strtotime($product['date_delivery']));
            $productName = trim($productName);
            //d(strlen("Frase.160/180.Silen.weiss.R.30/11"));
 ...
 echo "test4";
            for ($i = 0; $i < $product['product_quantity']; $i++) {
                $item = array(// 1.Item ...
                    ...
                    'Recipient' => array(
                        'Title' => substr(str_replace(' ', '.',$productName), 0, 35),
