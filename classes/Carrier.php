...
//		d($sql);
        ...
        $sql .= ' GROUP BY c.`name` ORDER BY c.`position` ASC';
...
//			AND z.`active` = 1';
        $sql = 'SELECT 
                    ...
                    FROM
                        `ps_carrier` c
                            inner join 
                        ps_planning_delivery_carrier_exception pde0 on pde0.id_carrier = c.id_carrier
                            LEFT JOIN
                        `ps_carrier_zone` cz ON (cz.`id_carrier` = c.`id_carrier`)
                            LEFT JOIN
                        `ps_zone` z ON (z.`id_zone` = '.(int)$id_zone.')
                    WHERE
                        c.`id_carrier` = '.(int)$id_carrier.' AND c.`deleted` = 0
                            AND c.`active` = 1
                            AND cz.`id_zone` = '.(int)$id_zone.'
                            AND z.`active` = 1
                            AND c.`id_carrier` NOT IN (SELECT 
                                id_carrier
                            FROM
                                (SELECT 
                                ...
                                WHERE
                                    1 = 1
                                        ...
                                        AND pde.`id_carrier` IN ('.(int)$id_carrier.')
                                GROUP BY pde.`id_planning_delivery_carrier_exception`
                                HAVING real_nb_commande >= `max_places`) t)';
//	    d($sql);
...
//		d($carrier_list);
        ...
        $npa  = Context::getContext()->cookie->npa;
        $allowedCarriers = [];
        foreach (Carrier::getAllowedCarrierByNpa($npa) as $carrier)
            $allowedCarriers[] = $carrier['id_carrier'];

        if(count($allowedCarriers) == 0)
            $allowedCarriers[] = Configuration::get('TUNNELVENTE_ID_CARRIER_POST');
        //todo

        //todo "93" is the id for the little sapin package
        if($product->id == 93)
            $allowedCarriers = array(Configuration::get('TUNNELVENTE_ID_CARRIER_POST'));

//        $carrier_list = array_intersect($carrier_list, $allowedCarriers);
...
//
//        d($npa, $id_zone, $region);

        return $allowedCarriers;
    }

    public static function getAllowedCarrier($cart_id, $product_id)
    {
        $sql = '
SELECT distinct
    c.`id_carrier`
FROM
    ...
    ps_planning_delivery_carrier_exception pde0 ON pde0.id_carrier = c.id_carrier
WHERE
    c.`deleted` = 0 AND c.`active` = 1
    AND c.`id_carrier` IN
    (
    SELECT wc.id_carrier FROM ps_gszonevente_npa n
join ps_gszonevente_region_warehouse rw on rw.id_gszonevente_region = n.id_gszonevente_region
join ps_warehouse_carrier wc on rw.id_warehouse = wc.id_warehouse
join ps_cart c on c.npa = n.name
where c.id_cart = '. $cart_id . ')
        ...
        FROM
            ...
            OR c.`id_carrier` IN
    (
select cr.id_carrier from ps_product_carrier pr
join ps_carrier cr on pr.id_carrier_reference = cr.id_reference
where pr.id_product = '. $product_id . ')
            ';
//	    d($sql);
...
WHERE
    ...
    AND c.`id_carrier` IN
    (
    SELECT z.id_carrier FROM ps_gszonevente_region z
join ps_gszonevente_npa n on n.id_gszonevente_region = z.id_gszonevente_region
where n.name = '. $npa .')
        AND c.`id_carrier` NOT IN (SELECT 
            id_carrier
        FROM
