    ...
    public function initContent() {
        ...
        if(Tools::isSubmit("npa")){
            ...
            if (!is_numeric($npa) || strlen($npa) != 4 || $npa < 1000 || $npa > 9999 ) {
                ...
                if(!count($dateDispo)){
                    $this->errors[] = Tools::displayError($this->trans('Tous nos jours de livraison de ce district sont complets pour cette année. Rendez-vous en 2022!', [], 'Modules.Tunnelventeabies.Type', $this->context->language->locale));
                     //activer npa
                    $steps->getStepByPosition(1)->setActive(true)
                        ->getStepDetailByPosition(1)->setActive(true);
                }else{
