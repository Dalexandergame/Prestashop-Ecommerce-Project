...
class TunnelVenteAbies extends Module
{
//    use Functions;
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->l('Tunnel de vente MyAbies');
        $this->description = $this->l('Module Tunnel de vente MyAbies');
        $module            = $this;
        $controllers       = array(
            1 => 'type',
            2 => 'taille',
        ...
        //etape 1
        $step1 = new StepAbies($module->l('Configuration du sapin'));
        $step1->addStepDetail(new StepDetailAbies($module->l('Code postal'), '#'));
        $step1->addStepDetail(new StepDetailAbies($module->l('Options'), "{$controllers[1]}"));
        $step1->addStepDetail(new StepDetailAbies($module->l('Type de pied'), "{$controllers[2]}"));
        $step1->addStepDetail(new StepDetailAbies($module->l('Livraison et recyclage'), "{$controllers[3]}"));
        $step1->addStepDetail(new StepDetailAbies($module->l('Mon Panier'), "{$controllers[4]}"));
        ...
        //etape 3
        $step3 = new Step($module->l('Confirmation et paiement'));
        $step3->addStepDetail(new StepDetail($module->l('Choisissez vos accesoires'), "{$controllers[6]}"));
        $step3->addStepDetail(new StepDetail($module->l('Little ecosapin'), "{$controllers[6]}"));
        $step3->addStepDetail(new StepDetail($module->l('Confirmation de commande'), "{$controllers[6]}"));*/

        $steps = StepsAbies::getInstance();
        $steps->addStep($step1);
        //->addStep($step2)
        //->addStep($step3);

        FrontAbies::$steps = $steps;
    }
...
//		Configuration::deleteByName('TUNNELVENTE_LIVE_MODE');
    ...
    protected function renderForm()
    {
        ...
        $helper->currentIndex  = AdminController::$currentIndex //$this->context->link->getAdminLink('AdminModules', false)
