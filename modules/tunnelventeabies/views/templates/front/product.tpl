...
<div class="image">
    {if $myLittelEcosapin}
        <p class="text-arrow-descp">{l s="Cliquez sur les flêches pour choisir la déco et la couleur du pot" mod='tunnelventeabies'}</p>
    ...
    <div id="image-block" class="clearfix">
            {if $product->new}
                    <span class="new-box">
                            <span class="new-label">{l s='New' mod='tunnelventeabies'}</span>
                    </span>
            {/if}
    ...
    {if $myLittelEcosapin}
        <div class="plus_infos">                                        
            <div class="pot text-center">
                <span>{l s='Couleur de pot' mod='tunnelventeabies'}</span>
                <span class="name"></span>
            </div>                                        
        </div>
        ...
        <div{if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || (isset($restricted_country_mode) && $restricted_country_mode) || $PS_CATALOG_MODE} class="unvisible"{/if}>
                <p id="add_to_cart" class="buttons_bottom_block no-print">
                        <button type="submit" name="Submit" class="exclusive">
                                <span>{if $content_only && (isset($product->customization_required) && $product->customization_required)}{l s='Customize' mod='tunnelventeabies'}{else}{l s='Add to cart' mod='tunnelventeabies'}{/if}</span>
                        </button>
                </p>
        </div>
...
<sapn class="text_p_contractuelle">{l s='Photo non contractuelle' mod='tunnelventeabies'}</sapn>
