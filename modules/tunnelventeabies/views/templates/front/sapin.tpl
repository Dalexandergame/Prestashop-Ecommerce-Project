<form action="{$urls.base_url}module/tunnelventeabies/{if $isSapinSwiss}pied{else}recyclage{/if}"
    ...
    <ul>
        {foreach from=$result item=sapin}
            <li data-id="{$sapin.id_product_attribute}">
                <input type="radio" name="sapin" data-idp="{$sapin.id_product}" data-price="{$sapin.price_ttc}"
                       value="{$sapin.id_product_attribute}" {if isset($id_product_sapin) and $sapin.id_product_attribute == $id_product_sapin } checked="" {/if}
                       id="sapin_{$sapin.id_product_attribute}"/>
    ...
    <div class="icon-tunnel">
        ...
        {*<h2>{l s="Choisissez l'essence de votre sapin" mod='tunnelventeabies'}</h2> *}
    </div>
    <div class="clear"></div>
    ...
    <div class="msg_info">
        <a title="Contactez-nous" target="_blank"
           href="{$urls.base_url}fr/contactez-nous">
            <h4>{l s="Demande spéciale, contactez nous ici" mod='tunnelventeabies'}</h4></a>
    </div>
...
<script type="text/javascript">
    var baseurl_tunnelvente = "{$urls.base_url}module/tunnelventeabies/taille?back=2";
</script>
{literal}
    <script type="text/javascript">
        $(function ($) {
            ...
            // trigger the submit event to skip this step
            $('form#form_sapin').submit();
        });
    </script>
{/literal}
