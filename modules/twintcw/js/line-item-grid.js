(function($) {
	...
	window['TwintCwLineItemGrid'] = {
		decimalPlaces: 2,
		currencyCode: 'EUR',
		
		init: function() {
			this.decimalPlaces = parseFloat($("#twintcw-decimal-places").val());
			this.currencyCode = $("#twintcw-currency-code").val();
			this.attachListeners();
		},
		
		attachListeners: function() {
			$(".twintcw-line-item-grid input.line-item-quantity").each(function() {
				TwintCwLineItemGrid.attachListener(this);
			});
			$(".twintcw-line-item-grid input.line-item-price-excluding").each(function() {
				TwintCwLineItemGrid.attachListener(this);
			});
			$(".twintcw-line-item-grid input.line-item-price-including").each(function() {
				TwintCwLineItemGrid.attachListener(this);
			});
		},
		...
		attachListener: function(element) {
			$(element).change(function() {
				TwintCwLineItemGrid.recalculate(this);
			});
		...
		recalculate: function(eventElement) {
			...
			var row = $('.twintcw-line-item-grid tr[data-line-item-index="' + lineItemIndex + '"]');
	...
	$(document).ready(function() {
		window['TwintCwLineItemGrid'].init();
	});
