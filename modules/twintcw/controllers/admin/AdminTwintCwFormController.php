 ...
 *      	   	 	  			 	  
 */
class AdminTwintCwFormController extends AdminController
{
	public function __construct() {
		$this->className = 'AdminTwintCwFormController';
		parent::__construct();
		$this->context->smarty->addTemplateDir($this->getTemplatePath());
		$this->tpl_folder = 'twintcw_form/';
		$this->bootstrap = true;
	}
...
require_once 'TwintCw/BackendFormRenderer.php';
	...
	private function createCronJobUrl() {
		$link = new Link();
		return $link->getModuleLink('twintcw', 'cron', array(), true);
	}
	...
	private function handleView() {
		...
		if ($form->isProcessable()) {
			$url = TwintCw::getAdminUrl('AdminTwintCwForm', array('save' => 'true', 'form' => $form->getMachineName()));
			$form = new Customweb_Form($form);
			$form->setTargetUrl($url)->setRequestMethod(Customweb_IForm::REQUEST_METHOD_POST);
		}

		$renderer = new TwintCw_BackendFormRenderer();
	...
	private function getBackendFormAdapter() {
		return TwintCw_Util::getBackendFormAdapter();
	}
