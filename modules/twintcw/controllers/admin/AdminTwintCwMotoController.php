 ...
 *      	   	 	  			 	  
 */
class AdminTwintCwMotoController extends AdminController
{
	public function __construct() {
		$this->className = 'AdminTwintCwMotoController';
		parent::__construct();
		$this->context->smarty->addTemplateDir($this->getTemplatePath());
		$this->tpl_folder = 'twintcw_moto/';
		$this->bootstrap = true;
	}
...
require_once 'TwintCw/FormRenderer.php';
		...
		if (isset($_GET['cw_transaction_id'])) {
			...
			$dbTransaction = TwintCw_Entity_Transaction::loadById($_GET['cw_transaction_id']);
			if ($dbTransaction->getTransactionObject()->isAuthorized()) {
				...
				$params['vieworder'] = '1';
				header('Location: ' . TwintCw::getAdminUrl('AdminOrders', $params));
				die();
			}
			else {
		...
		/* @var $moduleInstance TwintCw_IPaymentMethod */
		...
		$adapter = TwintCw_Util::getAuthorizationAdapter(Customweb_Payment_Authorization_Moto_IAdapter::AUTHORIZATION_METHOD_NAME);
		...
		if ($isSupported) {
			...
			if ($visibleFormFields !== null && count($visibleFormFields) > 0) {
				$renderer = new TwintCw_FormRenderer($moduleInstance->getPaymentMethodName());
