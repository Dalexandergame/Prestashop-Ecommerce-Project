 ...
 *
 */
class TwintCwCartModuleFrontController extends ModuleFrontController {
	public $ssl = true;
...
require_once 'TwintCw/Entity/Transaction.php';
		...
		if(!empty($errorTransactionId)){
			$errorTransaction = TwintCw_Entity_Transaction::loadById($errorTransactionId);
		...
		// external checkout
		$errorContextId = Tools::getValue('twintcw-context-id', null);
		if (!empty($errorContextId)) {
			$errorContext = TwintCw_Util::getEntityManager()->fetch('TwintCw_Entity_ExternalCheckoutContext', $errorContextId);
			/* @var $errorContext TwintCw_Entity_ExternalCheckoutContext */
