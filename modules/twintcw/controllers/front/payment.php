 ...
 *
 */
class TwintCwPaymentModuleFrontController extends ModuleFrontController {
	public $ssl = true;
...
require_once 'TwintCw/Entity/Transaction.php';
		...
		$this->addJS(_MODULE_DIR_ . 'twintcw/js/frontend.js');
		...
		if ($errorTransactionId !== null) {
			$errorDbTransaction = TwintCw_Entity_Transaction::loadById(intval($errorTransactionId));
		...
		try {
			$adapter = TwintCw_Util::getShopAdapterByPaymentAdapter($module->getAuthorizationAdapter($module->getOrderContext()));

			$createTransaction = true;
			$renderOnloadJs = true;
			if (isset($_REQUEST['ajaxAliasForm']) || TwintCw::getInstance()->isCreationOfPendingOrderActive()) {
