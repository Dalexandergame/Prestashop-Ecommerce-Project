 ...
 *
 */
class TwintCwIframeModuleFrontController extends ModuleFrontController {
	public $ssl = true;
...
require_once 'TwintCw/Adapter/IframeAdapter.php';
		...
		$wrapper = TwintCw_Util::getShopAdapterByPaymentAdapter(
				TwintCw_Util::getAuthorizationAdapter($transaction->getTransactionObject()->getAuthorizationMethod()));

		if (!($wrapper instanceof TwintCw_Adapter_IframeAdapter)) {
			throw new Exception("Expect 'TwintCw_Adapter_IframeAdapter'.");
		}

		$variables = array();
		$wrapper->prepareWithFormData($_REQUEST, $transaction);
		$variables['iframe'] = $wrapper->getIframe();
		TwintCw_Util::getEntityManager()->persist($transaction);
		$this->context->smarty->assign($variables);

		$this->setTemplate('module:twintcw/views/templates/front/iframe.tpl');
	}
}
