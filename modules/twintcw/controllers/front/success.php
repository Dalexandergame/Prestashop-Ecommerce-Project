...
class TwintCwSuccessModuleFrontController extends ModuleFrontController
{
	public $ssl = true;
...
require_once 'TwintCw/Entity/Transaction.php';
		...
		TwintCw_Util::redirectSetCookieIfRequired('success');
		...
		$dbTransaction = TwintCw_Entity_Transaction::loadById(intval($transactionId));
		...
		$timeoutUrl = $link->getModuleLink('twintcw', 'timeout', array('cw_transaction_id' => $dbTransaction->getTransactionId()), true);

		$failedUrl = $link->getModuleLink('twintcw', 'error', array(
		...
		while (true) {
			...
			$dbTransaction = TwintCw_Entity_Transaction::loadById(intval($transactionId), false);
