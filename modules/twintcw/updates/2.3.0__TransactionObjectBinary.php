...
class TwintCw_Migration_2_3_0 implements Customweb_Database_Migration_IScript{
	...
	public function execute(Customweb_Database_IDriver $driver) {
		...
		$result = $driver->query("SHOW COLUMNS FROM `" . _DB_PREFIX_ . "twintcw_transactions` LIKE 'transactionObjectBinary'");
		if ($result->getRowCount() <= 0) {
			$driver->query("ALTER TABLE  `" . _DB_PREFIX_ . "twintcw_transactions` ADD  `transactionObjectBinary` MEDIUMBLOB NULL")->execute();
		}
		
		
		$driver->query("CREATE TABLE IF NOT EXISTS `" . _DB_PREFIX_ . "twintcw_documents` (
