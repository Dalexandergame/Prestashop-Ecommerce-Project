...
require_once 'TwintCw/EntityManager.php';
require_once 'TwintCw/DatabaseLinkAccessor.php';
...
final class TwintCw_Util {
	...
	public static function getCheckoutCookieKey(Customweb_Payment_Authorization_IPaymentMethod $paymentMethod){
		return 'twintcw_checkout_id' . $paymentMethod->getPaymentMethodName();
	}
	...
	public static function redirectSetCookieIfRequired($action) {
		if($_SERVER['REQUEST_METHOD'] !== 'GET') { // ensure is only called when required, and no loop (Location: => GET)
			...
			$link = new Link();
			header('Location: ' . $link->getModuleLink('twintcw', $action, array('cw_transaction_id' => Tools::getValue('cw_transaction_id')), true));
			die();
		}
	}
	...
	public static function getPaymentCustomerContext($customerId){
		...
		if (!isset(self::$paymentCustomerContexts[$customerId])) {
			$entities = self::getEntityManager()->searchByFilterName('TwintCw_Entity_PaymentCustomerContext', 'loadByCustomerId',
					array(
			...
			if (count($entities) > 0) {
				...
				$context = new TwintCw_Entity_PaymentCustomerContext();
	...
	public static function persistPaymentCustomerContext(Customweb_Payment_Authorization_IPaymentCustomerContext $context){
		if ($context instanceof TwintCw_Entity_PaymentCustomerContext) {
	...
	public static function createContainer(){
		if (self::$container === null) {
			...
			$packages[] = 'TwintCw_LayoutRenderer';
			$packages[] = 'TwintCw_ExternalCheckoutService';
			$packages[] = 'TwintCw_EndpointAdapter';
			...
			$storageBackend = new Customweb_Storage_Backend_Database(self::getEntityManager(), self::getDriver(), 'TwintCw_Entity_Storage');
	...
	public static function getEndpointDispatcher(){
		return new Customweb_Payment_Endpoint_Dispatcher(self::createContainer()->getBean('TwintCw_EndpointAdapter'),
	...
	public static function getEntityManager(){
		if (self::$entityManager === null) {
			...
			self::$entityManager = new TwintCw_EntityManager(self::getDriver(), $cache);
		}
	...
	public static function getDriver(){
		if (self::$driver === null) {
			...
			$link = TwintCw_DatabaseLinkAccessor::getTwintCwLink($databaseInstance);
	...
	public static function getShopAdapterByPaymentAdapter(Customweb_Payment_Authorization_IAdapter $paymentAdapter){
		...
		$adapters = self::createContainer()->getBeansByType('TwintCw_Adapter_IAdapter');
		foreach ($adapters as $adapter) {
			if ($adapter instanceof TwintCw_Adapter_IAdapter) {
	...
	public static function getAssetResolver(){
		...
		if (!isset(self::$resolvers[$shopId])) {
			...
			$moduleUrl = $baseUrl . 'modules/twintcw';

			$currentTemplatePath = _PS_ALL_THEMES_DIR_ . $context->shop->getTheme() . '/modules/twintcw';
			$currentTemplateUrl = $baseUrl . '/themes/' . $context->shop->getTheme() . '/modules/twintcw';
	...
	public static function extractMailVariables(Customweb_Payment_Authorization_ITransaction $transaction){
		...
		$variables['{twintcw_transaction_id}'] = $transaction->getTransactionId();
		$variables['{twintcw_payment_id}'] = $transaction->getPaymentId();

		$variables['transaction_id'] = $transaction->getPaymentId();

		foreach ($transaction->getTransactionLabels() as $labelKey => $label) {
			$variables['{twintcw_label_' . $labelKey . '}'] = $label['value'];
		}
		...
		if ($transaction->getPaymentInformation() !== null) {
			...
			$variables['{twintcwpayment_information}'] = $paymentInformation;
			$variables['{twintcw_payment_information}'] = $paymentInformation;
			$variables['{twintcw_payment_information_txt}'] = Customweb_Core_Util_Html::toText($paymentInformation);
		}
		else {
			// set to empty, no conditionals possible in template
			$variables['{twintcwpayment_information}'] = $variables['{twintcw_payment_information}'] = $variables['{twintcw_payment_information_txt}'] = "";
		}
	...
	public static function getOrderCreationMessage($id_employee){
		...
		if (isset($id_employee) && $id_employee > 0) {
			...
			$message = TwintCw::translate('Manual order -- Employee:');
	...
	public static function createLineItemsFromCart(Cart $cart, $orderTotal, Customweb_Payment_Authorization_IPaymentMethod $paymentMethod = null){
		...
		if ($wrappingCosts > 0) {
			...
			$items[] = new Customweb_Payment_Authorization_DefaultInvoiceItem('wrapping', TwintCw::translate("Wrapping Fee"), $taxRate,
		...
		if (method_exists($cart, 'getFee') && $paymentMethod != null) {
			$cart->getFee('twintcw_' . $paymentMethod->getPaymentMethodName());
			$feeamount = $cart->feeamount;
			if ($feeamount != 0) {
				$items[] = new Customweb_Payment_Authorization_DefaultInvoiceItem('payment-fee', TwintCw::translate("Payment Fee"), 0,
