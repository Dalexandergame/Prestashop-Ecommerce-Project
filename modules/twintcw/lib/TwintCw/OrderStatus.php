...
class TwintCw_OrderStatus {
	
	const LEGACY_SETTING_KEY = 'PENDING_ORDER_STATUS_ID';
	
	const SETTING_KEY = 'TWINTCW_PENDING_STATE';
	
	private static $pendingOrderState = null;
	
	public static function getPendingOrderStatusId() {
		$result = TwintCw::getInstance()->getConfigurationValue(self::LEGACY_SETTING_KEY);
		if (!empty($result)) {
			return $result;
		}
		else {
	...
	public static function createPendingOrderState() {
		...
		$pendingOrderState->module_name = 'twintcw';
		
		foreach (Language::getLanguages() as $language) {
			$pendingOrderState->name[$language['id_lang']] = 'Awaiting Payment (TWINT)';
		}
		...
		TwintCw_OrderStatus::setPendingOrderStatusId($pendingOrderState->id);
