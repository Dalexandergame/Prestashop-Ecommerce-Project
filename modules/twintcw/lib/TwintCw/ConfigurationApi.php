...
class TwintCw_ConfigurationApi {
	...
	public function getConfigurationValue($key, $languageId = null) {
		...
		if (strpos($rs, self::ASSET_IDENTIFIER) === 0) {
			...
			$rs = TwintCw_Util::getAssetResolver()->resolveAssetStream($asset);
		}
	...
	private function getConfigurationKeyLegacy($key, $languageId = null) {
		$key = 'TWINTCW_' . $this->moduleId . '_' . $key;
		if ($languageId !== null) {
			$key .= '_' . $languageId;
		}
		...
		foreach ($fields as $fieldId => $field) {
			if ($field['type'] == 'orderstatus') {
				...
				if ($value instanceof Customweb_Core_Stream_Input_File) {
					$field['desc'] .= '<br />' . TwintCw::translate("Current: ") . $value->getFilePath();
				}
				$newFields[$fieldId] = $field;
				$newField = array(
					'type' => 'checkbox',
					...
					'values' => array(
						...
						'query' => array(
							'reset' => array(
								'val' => 'RESET',
								'name' => TwintCw::translate("Reset '@label' to default", array('@label' => $field['label'])),
								'id' => 'RESET',
							),
						),
					),
	...
	public function l($string, $specific = false) {
		return TwintCw::translate($string, $specific);
	}
