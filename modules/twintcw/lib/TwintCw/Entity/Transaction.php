...
/**
 * 
 * @Entity(tableName = 'twintcw_transactions')
 ...
 *
 */
class TwintCw_Entity_Transaction extends Customweb_Payment_Entity_AbstractTransaction
{
	...
	protected function authorize(Customweb_Database_Entity_IManager $entityManager) {
		...
		if (empty($orderId)) {
			...
			$GLOBALS['twintcw_successful_transaction_object'] = $transactionObject;
			$employeeId = null;
			...
			$message = TwintCw_Util::getOrderCreationMessage($employeeId);
			...
			$paymentMethod->validateOrder(
					...
					$message,
					TwintCw_Util::extractMailVariables($transactionObject),
					null,
					false,
					$customer->secure_key
			);
			...
			if (count($messages) > 0 && method_exists('Mail', 'sendMailMessageWithoutHook')) {
				...
				foreach ($messages as $message) {
					TwintCw_Util::attachTransactionToMailMessage($this->getTransactionObject(), $message);
	...
	public static function getGridQuery() {
		return 'SELECT * FROM ' . _DB_PREFIX_ . 'twintcw_transactions WHERE ${WHERE} ${ORDER_BY} ${LIMIT}';
	}
	...
	public static function loadById($id, $cache = true) {
		return TwintCw_Util::getEntityManager()->fetch('TwintCw_Entity_Transaction', $id, $cache);
	}
	...
	public static function getTransactionsByOrderId($orderId, $loadFromCache = true) {
		return TwintCw_Util::getEntityManager()->searchByFilterName('TwintCw_Entity_Transaction', 'loadByOrderId', array('>orderId' => $orderId), $loadFromCache);
	}
	...
	public static function getTransactionsByCartId($cartId, $loadFromCache = true) {
		return TwintCw_Util::getEntityManager()->searchByFilterName('TwintCw_Entity_Transaction', 'loadByCartId', array('>cartId' => $cartId), $loadFromCache);
	}
	...
	public static function getTransactionsByOriginalCartId($cartId, $loadFromCache = true) {
		return TwintCw_Util::getEntityManager()->searchByFilterName('TwintCw_Entity_Transaction', 'loadByOriginalCartId', array('>cartId' => $cartId), $loadFromCache);
	}
	...
	public static function getTransactionsByCartOrOrder($cartId, $orderId, $loadFromCache = true) {
		return TwintCw_Util::getEntityManager()->searchByFilterName('TwintCw_Entity_Transaction', 'loadByCartOrOrder', array('>cartId' => $cartId, '>orderId' => $orderId), $loadFromCache);
	}
