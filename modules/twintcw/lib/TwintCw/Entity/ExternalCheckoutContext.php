...
/**
 * @Entity(tableName = 'twintcw_external_checkout_contexts')
 ...
 */
class TwintCw_Entity_ExternalCheckoutContext extends Customweb_Payment_ExternalCheckout_AbstractContext {
	 ...
	 *
	 * @param Cart $cart
	 * @return TwintCw_Entity_ExternalCheckoutContext
	 */
	public function updateFromCart(Cart $cart, Customweb_Payment_Authorization_IPaymentMethod $paymentMethod = null){
		...
		$this->setCartUrl($link->getModuleLink('twintcw', 'cart', array(
			'twintcw-context-id' => $this->getContextId() 
		), true));
		...
		$this->setInvoiceItems(
				TwintCw_Util::createLineItemsFromCart($cart, $cart->getOrderTotal(true, Cart::BOTH, null, null, false), $paymentMethod));
		
		return $this;
	}

	protected function loadPaymentMethodByMachineName($machineName){
		return TwintCw::getInstanceByName('twintcw_' . $machineName);
	}
	...
	public static function getContextsByCartId($cartId, $loadFromCache = true){
		return TwintCw_Util::getEntityManager()->searchByFilterName('TwintCw_Entity_ExternalCheckoutContext', 
	...
	public static function getReusableContextByCartId($cartId, $loadFromCache = true){
		$result = TwintCw_Util::getEntityManager()->searchByFilterName('TwintCw_Entity_ExternalCheckoutContext', 
