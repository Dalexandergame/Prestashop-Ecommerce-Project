 ...
 * @Bean
 */
class TwintCw_ExternalCheckoutService extends Customweb_Payment_ExternalCheckout_AbstractCheckoutService {

	public function loadContext($contextId, $cache = true){
		return $this->getEntityManager()->fetch('TwintCw_Entity_ExternalCheckoutContext', $contextId, $cache);
	}

	public function renderShippingMethodSelectionPane(Customweb_Payment_ExternalCheckout_IContext $context, $errorMessages){
		if (!($context instanceof TwintCw_Entity_ExternalCheckoutContext)) {
			throw new Customweb_Core_Exception_CastException('TwintCw_Entity_ExternalCheckoutContext');
		}
		...
		return TwintCw_Util::getTemplateRenderer()->render($templateContext);
	}

	private function registerSmartyFunctions(){
		$smarty = TwintCw_Util::getTemplateSmartyContainer()->getSmartyInstance();
	...
	private function getShippingMethodNameFromContext(TwintCw_Entity_ExternalCheckoutContext $context){
	...
	public function authenticate(Customweb_Payment_ExternalCheckout_IContext $context, $emailAddress, $successUrl){
		...
		$externalCheckout = TwintCw::getInstance()->getConfigurationValue('external_checkout_account_creation');
		...
		$url = $link->getPageLink('authentication', true, null, 
				array(
					'twintcw-context-id' => $context->getContextId(),
	...
	protected function createTransactionContextFromContext(Customweb_Payment_ExternalCheckout_IContext $context){
		...
		if (!($paymentMethod instanceof TwintCw_IPaymentMethod)) {
			throw new Customweb_Core_Exception_CastException('TwintCw_IPaymentMethod');
		}
		$orderContext = new TwintCw_OrderContext(new Cart($context->getCartId()), 
				new TwintCw_PaymentMethodWrapper($paymentMethod));
	...
	public function getPossiblePaymentMethods(Customweb_Payment_ExternalCheckout_IContext $context){
		...
		foreach (PaymentModule::getInstalledPaymentModules() as $module) {
			if (strpos($module['name'], 'twintcw_') === 0) {
	...
	public function validateReviewForm(Customweb_Payment_ExternalCheckout_IContext $context, Customweb_Core_Http_IRequest $request){
		...
		if ($sessionContext->cookie->cart_total_amount != $cart->getOrderTotal()) {
			throw new Exception(TwintCw::translate("Cart content was modified."));
		}
		
		$parameters = $request->getParameters();
		if ((int) (Configuration::get('PS_CONDITIONS')) && (!isset($parameters['cgv']) || $parameters['cgv'] !== '1')) {
			throw new Exception(TwintCw::translate("You need to accept the general terms and conditions."));
		}
	}
	...
	private function updateMessage($messageContent, Cart $cart){
		if (!empty($messageContent)) {
			if (!Validate::isMessage($messageContent)) {
				throw new Exception(TwintCw::translate('Invalid message'));
			}
