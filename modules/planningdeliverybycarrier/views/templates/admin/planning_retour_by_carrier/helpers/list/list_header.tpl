	...
	$(function() {
		...
		$("#datepickerDatelivraison").datepicker({
			...
			dateFormat: 'yy-mm-dd'
                ...
                {/if}
                $('span.dateLivraison').text($("#datepickerDatelivraison").val());
	});
</script>
...
{include file="$path_weeklist/_dateRetour.tpl"}

{*{include file="$path_weeklist/exportorders.tpl"}*}

{include file="$path_weeklist/_weeklistRetour.tpl"}

<h2>{l s='All Deliveries' mod='planningdeliverybycarrier'}</h2>
			...
			<tr>
				<td style="vertical-align: bottom;">
					<span style="float: left;">
						...
						{/if}
						{l s='Page' mod='planningdeliverybycarrier'} <b>{$page}</b> / {$total_pages}
						{if $page < $total_pages}
						...
						{/if}
						| {l s='Display' mod='planningdeliverybycarrier'}
						...
						/ {$list_total} {l s='result(s)' mod='planningdeliverybycarrier'}
					</span>
					<span style="float: right;">
						<input type="submit" name="submitReset{$table}" value="{l s='Reset' mod='planningdeliverybycarrier'}" class="button" />
						<input type="submit" id="submitFilterButton{$table}" name="submitFilter" value="{l s='Filter' mod='planningdeliverybycarrier'}" class="button" />
					</span>
							...
							{if $shop_link_type}
								<th>
									{if $shop_link_type == 'shop'}
										{l s='Shop' mod='planningdeliverybycarrier'}
									{else}
										{l s='Group shop' mod='planningdeliverybycarrier'}
									{/if}
									<br />&nbsp;
								</th>
							{/if}
							{if $has_actions}
								<th class="center">{l s='Actions' mod='planningdeliverybycarrier'}{if !$simple_header}<br />&nbsp;{/if}</th>
							{/if}
						</tr>
 						{if !$simple_header}
									...
									{else}
										{if $params.type == 'bool'}
											<select onchange="$('#submitFilterButton{$table}').focus();$('#submitFilterButton{$table}').click();" name="{$table}Filter_{$key}">
												<option value="">--</option>
												<option value="1" {if $params.value == 1} selected="selected" {/if}>{l s='Yes' mod='planningdeliverybycarrier'}</option>
												<option value="0" {if $params.value == 0 && $params.value != ''} selected="selected" {/if}>{l s='No' mod='planningdeliverybycarrier'}</option>
											</select>
										{elseif $params.type == 'date' || $params.type == 'datetime'}
											{l s='From' mod='planningdeliverybycarrier'} <input type="text" class="filter datepicker" id="{$params.id_date}_0" name="{$params.name_date}[0]" value="{if isset($value.0)}$value.0{/if}"{if isset($params.width)} style="width:70px"{/if}/><br />
											{l s='To' mod='planningdeliverybycarrier'} <input type="text" class="filter datepicker" id="{$params.id_date}_1" name="{$params.name_date}[1]" value="{if isset($value.1)}$value.1{/if}"{if isset($params.width)} style="width:70px"{/if}/>
