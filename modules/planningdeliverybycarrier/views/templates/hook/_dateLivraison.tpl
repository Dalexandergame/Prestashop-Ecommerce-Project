<h2>{l s='Choisir une date de livraison' mod='planningdeliverybycarrier'}</h2>

<fieldset>
<form method="post" action="{$action}" class="form">
    {l s='Date de livraison' mod='planningdeliverybycarrier'} : <input type="text" name="datepickerDatelivraison" id="datepickerDatelivraison"/>
...
//                         .split(tmpColDelim).join(colDelim) ,
        ...
        function exportTableToCSV(me,$table, filename) {
            ...
            var $rows = $table.find('tr:has(td),tr:has(th)').not('.is_not_commande'),
                ...
                csv = '"' + $rows.map(function (i, row) {
                    var $row = $(row),
                        $cols = $row.find('td:eq(0),td:eq(1),td:eq(2),td:eq(3),td:eq(4),td:eq(5),td:eq(6),td:eq(7),td:eq(8),th:eq(0),th:eq(1),th:eq(2),th:eq(3),th:eq(4),th:eq(5),th:eq(6),th:eq(7),th:eq(8)');                        
                    return $cols.map(function (j, col) {
                        var $col = $(col),
                            text = $.trim($col.text());
                            if(j == 7){
