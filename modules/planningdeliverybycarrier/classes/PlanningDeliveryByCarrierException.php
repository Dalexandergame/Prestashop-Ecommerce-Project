...
*
*/
class PlanningDeliveryByCarrierException
{
	/**
	 * Add an exception
	...
	public static function get()
    {
        $sql = 'SELECT 
                ...
                (SELECT 
                    	...
                    	JOIN ps_carrier as ca ON (ca.id_carrier = so.id_carrier)
                        JOIN `ps_planning_delivery_carrier_exception` pde ON ca.id_carrier = pde.id_carrier
                    WHERE
                        1 = 1
                        AND so.`date_delivery` BETWEEN pde.`date_from` AND pde.`date_to`
	...
	public static function delete($id_exception)
	{
		if (!Validate::isUnsignedId($id_exception))
			die(Tools::displayError());
		return (Db::getInstance()->Execute('
		DELETE FROM `'._DB_PREFIX_.'planning_delivery_carrier_exception`
		WHERE `id_planning_delivery_carrier_exception` = '.(int)($id_exception)));
	}
